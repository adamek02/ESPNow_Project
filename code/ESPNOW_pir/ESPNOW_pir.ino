...
Pir_Sensor pir_sensor = {25, 0, 0};

void IRAM_ATTR isr() {
  TRIG = 1;
  trigger = 1;
	pir_sensor.detected = 1;
}
...
void setup() {
  ...
  pinMode(pir_sensor.PIN, INPUT_PULLUP);
  attachInterrupt(pir_sensor.PIN, isr, FALLING);
...
void loop() {
  PIN_STATUS = digitalRead(pir_sensor.PIN);

  if(PIN_STATUS == 1 && TRIG == 1){
    pir_sensor.detected = 0;
    TRIG = 0;
  }


  if(pir_sensor.detected == 0 && PIN_STATUS == 1){
    gas_trigger = 0;
...
//
  ...
  else if(pir_sensor.detected == 1 && PIN_STATUS == 0){
         gas_trigger = 1;
...
//
 ...
 if((pir_sensor.time > 1) && PIN_STATUS == 1){
        strcpy(myData.name, Dev_name_2);
        myData.data = pir_sensor.time;
        esp_now_send(broadcastAddress, (uint8_t *) &myData, sizeof(myData));
        pir_sensor.time = 0;
        delay(100);
        }
